# duplicated tests from neurow/test/neurow/jwt_auth_plug_authorization_header_test.exs
...
defmodule Neurow.JwtAuthPlugInterserviceAuthorizationHeaderTest do
  use ExUnit.Case
  use Plug.Test
  import JwtHelper
  ...
  test "prefers x-interservice-authorization header over authorization header if both are used",
       %{
         default_opts: opts
       } do
    jwt_interservice_auth_payload = valid_issuer_1_jwt_payload()
    jwt_auth_payload = valid_issuer_1_jwt_payload() |> Map.put("sub", "i_am_the_wrong_one")

    response =
      Neurow.JwtAuthPlug.call(
        conn(:get, "/test")
        |> put_jwt_token_in_req_header(
          jwt_interservice_auth_payload,
          @issuer_1_jwk_1,
          "x-interservice-authorization"
        )
        |> put_jwt_token_in_req_header(
    ...
    refute response.status
    refute response.resp_body
    assert response.assigns[:jwt_payload]["sub"] != "i_am_the_wrong_one"
    assert response.assigns[:jwt_payload]["sub"] == "record:123"
  end
